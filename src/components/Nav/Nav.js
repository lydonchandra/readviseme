...
export const NavRow = ( {row, displayFunc}) => {
  return (
    <TouchableOpacity onPress={ () => { 
      displayFunc(row.href) 
      } 
    }>
...
const _renderRow = (row, displayFunc) => {
  return (
    <NavRow row={row} displayFunc={displayFunc} />
  )
}

export const Nav = ( {isVisible, setIsVisible, displayFunc, toc = []} ) => {
  return (
    ...
    <View>
      <Modal animationType={"slide"} visible={isVisible} onRequestClose={ () => {} }>
        ...
        <FlatList
            ...
            data={toc}
            renderItem={(row) => {
              return _renderRow(row.item, (href) => {
                displayFunc(href)
                setIsVisible(false)
              })
            }}
